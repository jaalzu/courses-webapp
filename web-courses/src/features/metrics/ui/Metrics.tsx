// features/metrics/ui/MetricsPage.tsx
'use client';
...
export function Metrics() {
  // 1. Obtener datos de los stores
  const { users } = useUserList();
  const { courses } = useCourseStore();
  const { progress } = useProgressStore();
  
  // 2. Derivar métricas
  const metrics = deriveMetrics({ users, courses, progress });
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Métricas del Sistema</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">Panel de administración y estadísticas</p>
        </div>
